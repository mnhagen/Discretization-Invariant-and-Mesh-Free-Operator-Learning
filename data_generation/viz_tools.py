...
def anim_1D(x, y, pas_de_temps, pas_d_images, save=False,
             myxlim=(0, 4), myylim=(-4, 4),
             save_dir="/scratch/mnhagen/animations", filename="burgers_anim.mp4"):
    """
    Function allowing to display an animation based on calculation result with a given time step.
    ...
    def animate(i):
        ...
        ax.set_title(f"Wave form $u(x,t)$ after t = {i * pas_d_images * pas_de_temps:.2f} s",
                     fontsize=14)
        return line,

    anim = animation.FuncAnimation(fig, animate, init_func=init,
                                   frames=y.shape[1] // pas_d_images,
    ...
    if save:
        ...
        print("Animation not saved (set save=True to save).")
