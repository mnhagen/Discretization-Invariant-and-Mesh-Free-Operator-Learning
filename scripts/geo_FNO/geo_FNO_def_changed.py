...
class SpectralConv2d(nn.Module):
    def __init__(
        ...
        s2=32,
        L=(1.0, 1.0),
        spectral_enforce_real=False,
        spectral_tie_weights=False,
    ):
        ...
        self.L1 = float(L[0])
        self.L2 = float(L[1])
        self.spectral_enforce_real = spectral_enforce_real
        self.spectral_tie_weights = spectral_tie_weights

        # Optional diagnostics / regularization hooks.
        self.track_imag_energy = False
        self.last_imag_energy = None
    ...
    def _effective_weights(self):
        ...
        if self.spectral_enforce_real:
            # Keep tensors complex-typed for complex einsum while zeroing imaginary part.
            w1 = w1.real + 0j
            w2 = w2.real + 0j

        if self.spectral_tie_weights:
            w2 = w1

        return w1, w2

    def forward(self, u, x_in=None, x_out=None, iphi=None, code=None, w_in=None, w_out=None):
        batchsize = u.shape[0]
        ...
        if x_in == None:
            ...
            u_ft = self.fft2d(u, x_in, iphi, code, w_in=w_in)
            s1 = self.s1
            s2 = self.s2
        ...
        factor1 = self.compl_mul2d(u_ft[:, :, :self.modes1, :self.modes2], w1)
        factor2 = self.compl_mul2d(u_ft[:, :, -self.modes1:, :self.modes2], w2)
        ...
        if x_out == None:
            ...
            u = self.ifft2d(out_ft, x_out, iphi, code, w_out=w_out)

        return u

    def fft2d(self, u, x_in, iphi=None, code=None, w_in=None):
        # u (batch, channels, n)
        ...
        # Y (batch, channels, m1, m2)
        # Optional quadrature weighting for irregular meshes.
        # Normalize weights to mean 1 so training scale remains comparable.
        u = u + 0j
        if w_in is None:
            Y = torch.einsum("bcn,bnxy->bcxy", u, basis)
        else:
            w = w_in.to(u.device).to(u.real.dtype)
            w = w / (torch.mean(w, dim=1, keepdim=True) + 1e-12)
            Y = torch.einsum("bcn,bn,bnxy->bcxy", u, w, basis)
        return Y

    def ifft2d(self, u_ft, x_out, iphi=None, code=None, w_out=None):
        ...
        # Consistent inverse normalization for the (m1, m2) spectral grid.
        Y = Y / (m1 * m2)
        if self.track_imag_energy:
            self.last_imag_energy = torch.mean(Y.imag ** 2)
        Y = Y.real
        return Y
...
class FNO2d(nn.Module):
    def __init__(
        ...
        self.conv0 = SpectralConv2d(
            self.width, self.width, self.modes1, self.modes2, s1, s2, L=L,
            spectral_enforce_real=spectral_enforce_real, spectral_tie_weights=spectral_tie_weights
        )
    ...
    def forward(self, u, code=None, x_in=None, x_out=None, iphi=None, x_in_w=None, x_out_w=None):
        ...
        uc1 = self.conv0(u, x_in=x_in, iphi=iphi, code=code, w_in=x_in_w)
        uc3 = self.b0(grid)
        uc = uc1 + uc3
        uc = F.gelu(uc)
        ...
        u = self.conv4(uc, x_out=x_out, iphi=iphi, code=code, w_out=x_out_w)
